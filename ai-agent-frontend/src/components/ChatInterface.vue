<template>
  <div class="chat-container">
    ...
    <div class="chat-messages" ref="messagesContainer">
      <div v-if="messages. gth === 0" class="empty-chat">
...
export default {
  ...
  setup(props) {
    ...
    // 打字机效果
    const typeNextChar = () => {
      if (currentTypingMessageIndex.value >= 0 && currentTypingMessageIndex.value < messages.value. gth) {
        const currentMessage = messages.value[currentTypingMessageIndex.value];
        if (!currentMessage.isUser && displayText.value. gth < fullText.value. gth) {
        // 显示下一个字符
        displayText.value = fullText.value.substring(0, displayText.value. gth + 1);
        // 继续打字
    ...
    const startTyping = (text, messageIndex) => {
      ...
      isTyping.value = true;
      // 记录正在打字的消息索引
      currentTypingMessageIndex.value = messageIndex !== undefined ? messageIndex : messages.value. gth - 1;
      // 开始打字
      clearTimeout(typingTimer);
      typingTimer = setTimeout(typeNextChar, typingSpeed.value);
    };
        ...
        const updateResponse = (text, isNewStep = false) => {
          ...
          if (isNewStep && text.trim()) {
            ...
            if (isPdfMessage) {
              ...
              // 如果当前不在打字状态，启动打字效果
              if (!isTyping.value) {
                startTyping(text, messages.value. gth - 1);
              } else if (currentTypingMessageIndex.value === messages.value. gth - 1) {
    ...
    const shouldApplyTypingEffect = (message, index) => {
      ...
      // 如果是最后一条消息且正在打字中
      if (index === messages.value. gth - 1 && currentTypingMessageIndex.value === index) {
