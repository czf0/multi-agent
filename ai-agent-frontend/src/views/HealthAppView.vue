    ...
    const sendMessage = (message) => {
      ...
      const eventSource = connectToHealthChat(
        ...
        (data) => {
          ...
          // 更新步骤数组
          if (steps. gth > currentSteps.value. gth) {
            // 有新的步骤
            for (let i = currentSteps.value. gth; i < steps. gth; i++) {
              // 为每个新步骤创建一个新的响应
              const stepMessage = {
                content: steps[i],
                isUser: false,
            ...
            const lastStepIndex = messages.value.findIndex(
              msg => !msg.isUser && msg.content.includes(currentSteps.value[currentSteps.value. gth - 1].substring(0, 20))
            );
            
            if (lastStepIndex !== -1) {
              messages.value[lastStepIndex].content = steps[steps. gth - 1];
            }
