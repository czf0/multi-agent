...
import com.czf.aiagent.tools.WebSearchTool;
import lombok.Data;
...
public abstract class BaseAgent {
    ...
    // 状态与执行控制
    private AgentStatusEnum state = AgentStatusEnum.IDLE;
    ...
    public String run(String userPrompt) {
        // 校验
        if (state != AgentStatusEnum.IDLE || StrUtil.isBlank(userPrompt)) {
            throw new RuntimeException("无法执行: " + (state != AgentStatusEnum.IDLE ? "状态错误" : "空提示词"));
        }
        
        // 执行
        state = AgentStatusEnum.RUNNING;
        messageList.add(new UserMessage(userPrompt));
        List<String> results = new ArrayList<>();
        
        try {
            // 步骤循环
            for (int i = 0; i < maxSteps && state != AgentStatusEnum.FINISHED; i++) {
                currentStep = i + 1;
            ...
            if (currentStep >= maxSteps) {
                state = AgentStatusEnum.FINISHED;
                results.add("终止: 达到最大步骤 (" + maxSteps + ")");
            }
            
            return String.join("\n", results);
        } catch (Exception e) {
            state = AgentStatusEnum.ERROR;
            log.error("执行错误", e);
    ...
    public SseEmitter runStream(String userPrompt) {
        ...
        CompletableFuture.runAsync(() -> {
            try {
                // 校验
                if (state != AgentStatusEnum.IDLE || StrUtil.isBlank(userPrompt)) {
                    String error = "错误: " + (state != AgentStatusEnum.IDLE ? "无法从状态运行: " + state : "空提示词");
                    emitter.send(error);
                    emitter.complete();
                    return;
                }
        ...
        emitter.onCompletion(() -> {
            if (state == AgentStatusEnum.RUNNING) {
                state = AgentStatusEnum.FINISHED;
            }
            cleanup();
            log.info("SSE连接完成");
